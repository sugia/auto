...
def smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00001):
    ...
    # Enter code here
    #
    newpath = [[path[i][j] for j in xrange(len(path[0]))] for i in xrange(len(path))]
    flag = True
    while flag:
        flag = False
        for i in xrange(len(path)):
            for j in xrange(len(path[i])):
                delta = weight_data * (path[i][j] - newpath[i][j]) + weight_smooth * (newpath[(i-1)%len(path)][j] + newpath[(i+1)%len(path)][j] - 2.0 * newpath[i][j])
                newpath[i][j] += delta
                if delta > tolerance:
                    flag = True
    return newpath
